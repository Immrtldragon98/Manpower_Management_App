...
export async function GET(){
 ...
 const company=admin.companyId?await query("SELECT id,code,name,timezone FROM companies WHERE id=$1",[admin.companyId]):await query("SELECT id,code,name,timezone FROM companies ORDER BY id LIMIT 1");
...
export async function POST(request:Request){
 ...
 if(body.action==="company"){
  ...
  try{const companyId=admin.companyId||(await query("SELECT id FROM companies ORDER BY id LIMIT 1")).rows[0].id;await query("UPDATE companies SET name=$1,code=$2 WHERE id=$3",[p.data.name,p.data.code.toUpperCase(),companyId]);return Response.json({ok:true})}catch(e){return Response.json({error:String(e).includes("23505")?"Company code is already used.":"Unable to save company."},{status:400})}
 }
 ...
 try{await query("INSERT INTO organization_units(company_id,unit_type,name,parent_id) VALUES($1,$2,$3,$4)",[companyId,p.data.type,p.data.name,p.data.parentId]);return Response.json({ok:true},{status:201})}catch(e){return Response.json({error:String(e).includes("23505")?"This level already exists.":"Unable to add organization level."},{status:400})}
}
