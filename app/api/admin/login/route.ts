...
export async function POST(request:Request){
 ...
 const p=parsed.data,cfg=process.env,secret=cfg.SESSION_SECRET||"",key=loginKey("staff",p.companyCode||"PRIMARY",p.email),allowed=await loginAllowed(key);
 ...
 const r=await query('SELECT s.id,s.company_id AS "companyId",s.role,s.password_salt AS "passwordSalt",s.password_hash AS "passwordHash" FROM staff_accounts s JOIN companies c ON c.id=s.company_id WHERE c.code=$1 AND LOWER(s.email)=LOWER($2) AND s.active=TRUE AND c.active=TRUE',[p.companyCode.toUpperCase(),p.email]);const row=r.rows[0];
 if(!row||await hashPin(p.password,row.passwordSalt)!==row.passwordHash){const locked=await recordFailure(key);return Response.json({error:locked?"Account locked for 15 minutes after repeated attempts.":"Company code, email or password is incorrect."},{status:locked?429:401})}
